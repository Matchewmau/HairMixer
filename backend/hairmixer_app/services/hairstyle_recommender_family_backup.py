...
This service loads the hairmixer_model and provides
hairstyle recommendations based on user preferences.

Key Features:
    - Uses Random Forest classifier (203 specific hairstyles - DIRECT prediction)
    ...
    - Direct hairstyle prediction (not family-based)
"""
import pickle
import logging
import json
...
class HairstyleRecommender:
    ...
    def _load_model(self):
        """
        Load the trained hairstyle model from disk.
        
        Loads 'hairmixer_model.pkl' (direct 203-class predictor) and
        'label_encoder.json' to map numeric predictions to hairstyle names.
        
        Sets:
            self.model: The loaded RandomForestClassifier
            self.label_encoder: Dictionary mapping labels to hairstyle names
            self.model_loaded: True if successful, False otherwise
        """
        try:
            # Load main model
            model_path = Path(__file__).parent.parent / 'ml' / 'models' / 'hairmixer_model.pkl'
            ...
            # Load label encoder
            encoder_path = Path(__file__).parent.parent / 'ml' / 'models' / 'label_encoder.json'
            ...
            with open(encoder_path, 'r') as f:
                ...
                self.label_encoder = {int(k): v for k, v in encoder_data['label_to_name'].items()}
            
            self.model_loaded = True
            logger.info(
                f"Direct hairstyle prediction model loaded: "
            ...
            if hasattr(self.model, 'classes_'):
                logger.info(
                    f"Model predicts {len(self.model.classes_)} specific hairstyles (direct)"
                )
    ...
    def get_top_recommendations(
        ...
        """
        Get top N hairstyle recommendations using the trained Random Forest model.
        
        This method implements a smart recommendation strategy:
        1. Model predicts top families with confidence scores (26 classes)
        2. For each family, retrieve multiple hairstyles from database
        3. Distribute the 10 recommendations across high-confidence families
        4. Return exactly 10 hairstyles with their ML-predicted scores
        
        Args:
            preferences: User preference dictionary with 21 features
            top_n: Number of recommendations to return (default: 10)
            
        Returns:
            List of exactly 10 recommended hairstyles with ML scores
            
        Raises:
            Exception: If model fails, returns empty list (no fallback)
        """
